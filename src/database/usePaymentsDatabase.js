...
export function usePaymentsDatabase() {
  ...
  async function getPayments(page) {
    try {
      const payments = await database.getAllAsync(`SELECT p.*, u.nome FROM payments p, users u WHERE u.id = p.user_id ORDER BY data_pagamento DESC LIMIT 7 OFFSET ${page * 7}`);
      return payments;
    } catch (error) {
      console.log(error);
      throw error;
